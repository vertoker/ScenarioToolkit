...
#endif
    public class TimerSystem : BaseScenarioAsyncStateSystem<TimerState>
    {
        ...
        public bool StartTimer(StartTimer component)
        {
            ...
            if (AssertLog.Above<StartTimer>(component.RealTime, 0, nameof(component.View))) return false;
            if (AssertLog.Above<StartTimer>(component.InGameTime, 0, nameof(component.View))) return false;
            ...
            if (source.Debug)
                Debug.Log($"Timer {component.ID} <b>started</b>. " +
                          ...
                          $"RealTime: {component.RealTime}", component.View?.gameObject);
        ...
        private void StopTimer(Entry entry, bool cancelled)
        {
            ...
            if (entry.Component.DisableOnEnd)
                entry.Component.View?.Close();
            entries.Remove(entry.Component.ID);

            State.PausedTimers.Remove(entry.Component.ID);
            State.RunningTimers.Remove(entry.Component.ID);
            ...
            if (source.Debug)
                Debug.Log($"Timer {entry.Component.ID} " +
                          ...
                          $"RealTime: {entry.Component.RealTime}", entry.Component.View?.gameObject);
        }
