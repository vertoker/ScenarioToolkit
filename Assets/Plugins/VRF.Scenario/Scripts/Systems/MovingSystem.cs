...
#endif
    public class MovingSystem : BaseScenarioAsyncStateSystem<MovingState>
    {
        ...
        protected override void ApplyState(MovingState state)
        {
            foreach (var (transform, data) in state.Transforms)
            {
                AnimateTransform(
                    transform, 
                    data.StartTransformInfo, 
                    data.EndTransformInfo,
                    data.Seconds,
                    data.AnimatePosition,
                    data.AnimateRotation,
                    data.AnimateScale,
                    data.Ease,
                    (float)data.GetPassedTime());
            }
        }
        ...
        private void AnimateTransform(AnimateTransform component)
        {
            ...
            if (State.Transforms.ContainsKey(component.MovingObject))
            {
                State.Transforms[component.MovingObject] = new MovingState.Data(component);
            }
            else
            {
                State.Transforms.Add(component.MovingObject, new MovingState.Data(component));
            }
        }
