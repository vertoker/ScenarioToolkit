// Специальные флаги (которые Unity не хочет добавлять) ускоряющие IL2CPP код
// За счёт отключения NullChecks и ArrayBoundsChecks
// Оригинальный скрипт изменён на internal, чтобы работало в рамках каждого модуля отдельно
// Если не даёт скомпилироваться - смело удаляйте (не делать push без обсуждения)
